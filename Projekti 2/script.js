...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Funktio hakee teatterit FinnKino APIsta
    function fetchTheaters() {
      // Tehdään AJAX-pyyntö teattereille
      const xhr = new XMLHttpRequest();
      xhr.open('GET', 'http://www.finnkino.fi/xml/TheatreAreas/', true);
      xhr.onload = function() {
        if (xhr.status === 200) {
...
function fetchMovies() {
    ...
    // Tehdään AJAX-pyyntö elokuville
    const xhr = new XMLHttpRequest();
    xhr.open('GET', `http://www.finnkino.fi/xml/Schedule/?area=${theaterId}`, true);
    xhr.onload = function() {
      if (xhr.status === 200) {
        const xmlDoc = xhr.responseXML;
